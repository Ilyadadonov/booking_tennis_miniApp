...
export function BookingConfirmSheet({ slot, onClose }: BookingConfirmSheetProps) {
  ...
  const inv = () => { queryClient.invalidateQueries({ queryKey: ['slots'] }); queryClient.invalidateQueries({ queryKey: ['my-bookings'] }) }
  const bookMutation = useMutation({ mutationFn: () => createBooking(slot!.id), onSuccess: () => { hapticFeedback.notification('success'); inv(); onClose() }, onError: () => hapticFeedback.notification('error') })
  const cancelMutation = useMutation({ mutationFn: () => cancelBooking(slot!.booking_id!), onSuccess: () => { hapticFeedback.notification('success'); inv(); onClose() }, onError: () => hapticFeedback.notification('error') })
  if (!slot) return null
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.sheet} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.actions}>
          {isMine
            ? <button className={`${styles.btn} ${styles.btnDanger}`} onClick={() => cancelMutation.mutate()} disabled={isLoading}>{cancelMutation.isPending ? 'Отменяем...' : 'Отменить бронь'}</button>
            : <button className={`${styles.btn} ${styles.btnPrimary}`} onClick={() => bookMutation.mutate()} disabled={isLoading}>{bookMutation.isPending ? 'Бронируем...' : 'Забронировать'}</button>
          }
          <button className={`${styles.btn} ${styles.btnSecondary}`} onClick={onClose} disabled={isLoading}>Закрыть</button>
        </div>
      </div>
    </div>
  )
