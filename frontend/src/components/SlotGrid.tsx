...
export function SlotGrid({ courts, onSlotTap }: SlotGridProps) {
  if (courts.length === 0) {
    return <div className={styles.empty}>Нет доступных слотов на этот день</div>
  }
      ...
      {courts.map(({ court, slots }) => (
        <div key={court.id} className={styles.court}>
          <div className={styles.courtHeader}>
            ...
            <span className={styles.courtDesc}>{court.description}</span>
          </div>
          <div className={styles.slots}>
            {slots.map((slot) => {
              const isMine = slot.status === 'occupied' && slot.booking_id !== undefined
              ...
              return (
                <button
                  ...
                  onClick={() => slot.status === 'free' || isMine ? onSlotTap(slot) : undefined}
                  disabled={slot.status === 'occupied' && !isMine}
                >
                  <span className={styles.time}>{slot.time_start.slice(0, 5)}</span>
                  <span className={styles.badge}>
                    {isMine ? 'Я' : slot.status === 'occupied' ? '✕' : ''}
                  </span>
                </button>
              )
            })}
